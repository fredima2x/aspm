...
class ClientHandler(threading.Thread):
    def __init__(self, client_socket, addr, clients, clients_lock, clients_data, clients_data_lock):
        ...
        self.clients_data_lock = clients_data_lock

    def run(self):
        self.logger.info(f"Verbindung von {self.addr} akzeptiert")
        ...
        while True:
            try:
                ...
                self.logger.error(f"Socket-Fehler bei Client {self.addr}: {e}")
                break
            except Exception as e:
                self.logger.error(f"Unerwarteter Fehler bei Client {self.addr}: {e}")
                break
            if not data:
                break
            message = data.decode()
            ...
            if ready_message[0] == "send_creds":
                self.logger.info("Got send_creds Request!")
                if len(ready_message) < 3:
                    self.logger.error("Incomplete send_creds request")
                    continue
                username = ready_message[1]
                password = ready_message[2]
                self.verify_user(username, password)
            elif ready_message[0] == "send_newuser":
                self.logger.info("Got send_newuser Request!")
                if len(ready_message) < 3:
                    self.logger.error("Incomplete send_newuser request")
                    continue
                username = ready_message[1]
                password = ready_message[2]
            ...
            # Commands for verified users
                ...
                try:
                    if ready_message[0] == "get_chats":
                        ...
                        if len(ready_message) < 3:
                            self.logger.error("Incomplete add_user_to_chat request")
                            continue
                        ...
                        if len(ready_message) < 3:
                            self.logger.error("Incomplete remove_user_from_chat request")
                            continue
                        chat_id = int(ready_message[1])
                        identifier = ready_message[2]
                        ...
                        if len(ready_message) < 4:
                            self.logger.error("Incomplete get_messages request")
                            continue
                        chat_id = int(ready_message[1])
                        ...
                        if len(ready_message) < 3:
                            self.logger.error("Incomplete send_message request")
                            continue
                        chat_id = int(ready_message[1])
                        ...
                        if len(ready_message) < 2:
                            self.logger.error("Incomplete new_chat request")
                            continue
                        chat_name = ready_message[1]
                        self.new_chat(chat_name)
                    elif ready_message[0] == "delete_account":
                        self.delete_account()
                    elif ready_message[0] == "delete_chat":
                        if len(ready_message) < 2:
                            self.logger.error("Incomplete delete_chat request")
                            continue
                        chat_id = int(ready_message[1])
        ...
        self.clients_lock.release()
        self.clients_data_lock.acquire()
        if self.client_socket in self.clients_data:
            del self.clients_data[self.client_socket]
        self.clients_data_lock.release()
        self.logger.info(f"Verbindung von {self.addr} geschlossen!")
    ...
    def new_user(self, username, password):
        if username is not None and password is not None:
            ...
            if user_id:
                ...
                self.clients_data_lock.acquire()
                self.clients_data[self.client_socket] = {"user_id": user_id}
                self.clients_data_lock.release()
                self.client_socket.sendall("verified".encode())
                self.logger.info(f"New user {username} created and verified")
            else:
    ...
    def verify_user(self, username, password):
        if username is not None and password is not None:
            ...
            if verify_status is None:
                ...
                self.logger.debug(f"Wrong Creds! {self.addr}")
            else:
                self.user_id = verify_status
                self.verified_user = True
                self.logger.info(f"Verified user {self.addr}!")
                self.clients_data_lock.acquire()
                self.clients_data[self.client_socket] = {"user_id": self.user_id}
                self.clients_data_lock.release()
                self.client_socket.sendall("verified".encode())
        else:
            self.client_socket.sendall("invalid".encode())
    def get_chats(self):
        ...
        self.logger.debug(f"get_user_chats returned {chats}")
        self.logger.debug(f"User {self.addr} requested chat list, sending {len(chats) if chats else 0} chats")
        if chats is not None and len(chats) > 0:
            ...
            self.logger.debug(f"Sent chat list to {self.addr}: {chats_string}")
        else:
            self.client_socket.sendall("None".encode())
            self.logger.debug(f"No chats found for user {self.addr}, sent 'None'")
    def add_user_to_chat(self, chat_id, identifier):
        ...
        if success:
            ...
            self.logger.info(f"User {identifier} added to chat {chat_id}")
        else:
            self.client_socket.sendall("add_user_failed".encode())
            self.logger.error(f"Failed to add user {identifier} to chat {chat_id}")
    def remove_user_from_chat(self, chat_id, identifier):
        ...
        if success:
            ...
            self.logger.info(f"User {identifier} removed from chat {chat_id}")
        else:
            self.client_socket.sendall("remove_user_failed".encode())
            self.logger.error(f"Failed to remove user {identifier} from chat {chat_id}")
    ...
    def send_message(self, chat_id, message_content):
        try:
            ...
            self.logger.error(f"Fehler beim Speichern der Nachricht: {e}")
            self.client_socket.sendall("message_save_failed".encode())
    def new_chat(self, chat_name):
        self.logger.debug(f"User {self.addr} requested new chat creation with name '{chat_name}'")
        chat_id = self.db.new_chat(self.user_id)
        if chat_id is not None:
            self.logger.debug(f"Chat '{chat_name}' mit ID {chat_id} erfolgreich für UserID: {self.user_id} erstellt")
            self.client_socket.sendall(f"chat_created;{chat_id}".encode())
            self.logger.debug(f"UserID: {self.user_id} erfolgreich zum Chat '{chat_name}' hinzugefügt")
        else:
    ...
    def delete_chat(self, chat_id):
        try:
            ...
            self.logger.error(f"Error fetching chat members for chat {chat_id}: {e}")
        ...
        for m in member:
            if m == self.user_id:
                ...
                break
        if not is_in_chat:
            self.client_socket.sendall("not_in_chat".encode())
